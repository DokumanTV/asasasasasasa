...
exports.run = (client, message, args) => {
  const kayıtlı = message.guild.roles.find(r => r.id === "711600932139303072"); //buraya kadın rolünüzün id'sini koyun
  const male = message.guild.roles.find(r => r.id === "711659514407157861"); //buraya kadın rolünüzün id'sini koyun
  const smale = message.guild.roles.find(r => r.id === "711600932818780200"); //buraya kadın rolünüzün id'sini koyun
  ...
  if(!message.member.roles.array().filter(r => r.id === "711600922542866532")[0]) { //buraya kayıt sorumlusu rolünün id'sini giriniz. SUNUCU AYARLARINDAN kopyalayın.
    ...
    .setDescription(`<a:pembetik:711638772340752435> **<@${c.user.id}>** adlı kişiye **<@&${kayıtlı.id}>** rolü verildi. !`)
    .setColor("0xff5cf3")
    log.send(embed)
